...
#########################################################################
#### We can't share GPS locations, but below are the data prep steps ####
#### that led to the aspatial movement data provided to replicate    ####
#### the movement summaries and plots in the MS                      ####
#########################################################################
...
# 
# trk.aspatial <- setDT(trk)[,.(id, WolfID, PackID, pop, year, sl_, ta_, season)]
# saveRDS(trk.aspatial, file.path(derived, "move_dat_aspatial.RDS"))

trk <- readRDS(file.path(derived, "move_dat_aspatial.RDS"))
